...
export const patchProfileRouteHandler = async (req, res) => {
  ...
  const { first_name, last_name, phone, address, email } = req.body.data.attributes;
  ...
  let foundUser = await UserModel.findOne({ email: currentDataOfUser.email });
  ...
  if (foundUser == null) {
    ...
    if ((newPassword && newPassword.length < 7) || newPassword != confirmPassword) {
      ...
      try {
        ...
        const response = await UserModel.updateOne( { email: foundUser.email }, { $set :{ "first_name": first_name, "last_name":last_name, "phone":phone, "address":address, "email": email, "password": hashPassword } });
      } catch (err) {
        console.error(err);
      }
      const sentData = {
      ...
      console.log("User found but but no password change.");
      try {
        const response = await UserModel.updateOne( { email: foundUser.email }, { $set :{ "first_name": first_name, "last_name":last_name, "phone":phone, "address":address, "email": email} });
      } catch (err) {
        console.error(err);
      }
      const sentData = {
