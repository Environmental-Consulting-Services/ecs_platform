...
import { InspectionTemplateModel } from "./schema/inspectionTemplate.schema";
...
export const getInspectionTemplateRoute = async (req, res) => {
  ...
  var foundInspectionTemplate;
  try{
     foundInspectionTemplate = await InspectionTemplateModel.findOne({ _id: inspectionTemplateId }).select(fieldsInspectionTemplate);
  }catch(err){
    console.error(err);
  }
...
export const editInspectionTemplateRoute = async (req, res) => {
  const inspectionTemplateId = req.params.id;
  const { type,  status, name, items, project, company  } = req.body.data.attributes;
  ...
  if (name) {
    foundInspectionTemplate.name = name;
  }
  if (status) {
    foundInspectionTemplate.status = status;
  }
  if (items) {
    foundInspectionTemplate.items = items;
  }
  if (project) {
    foundInspectionTemplate.project = project;
  }
  if (company) {
    foundInspectionTemplate.company = company;
  }
  foundInspectionTemplate.updated_at = Date.now();
  foundInspectionTemplate.save();
  const sentData = {
    data: {
      type: "inspectionTemplates",
