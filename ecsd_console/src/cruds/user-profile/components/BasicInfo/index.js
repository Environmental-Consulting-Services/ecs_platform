...
function BasicInfo({ user, isDemo }) {
  const [info, setInfo] = useState({ first_name: "", last_name: "", phone: "", address: "", email: "" });
  ...
  const submitHandler = async (e) => {
    ...
    if (info.first_name.trim().length === 0) {
      setErrors({ ...errors, first_nameError: true });
      return;
    }

    if (info.last_name.trim().length === 0) {
      setErrors({ ...errors, last_nameError: true });
      return;
    }
